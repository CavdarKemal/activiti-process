...
for i in $(seq 1 60); do
    sleep 5
    TABLES=$(PGPASSWORD=postgres psql -h activiti5-db -U postgres -d activiti -tAc "SELECT count(*) FROM information_schema.tables WHERE table_name='act_id_user'" 2>/dev/null)
    if [ "$TABLES" = "1" ]; then
        echo "Activiti-Tabellen gefunden. Lege User an..."
        PGPASSWORD=postgres psql -h activiti5-db -U postgres -d activiti -f /usr/local/tomcat/init-users.sql
        echo "User angelegt!"
        break
    fi
    echo "  Warte... ($i)"
